...
import { FormField, FormRow, FormActions } from './index';
import { Input } from '../Input';
import { Select } from '../Select';

const NAME = 'FormField 字段容器';
const DESC = 'label/required/error 包装';
const PROMPT = '【组件引用】FormField 字段容器（表单）\n框架：React + Tailwind v4\n用法：import { FormField } from "../../component-templates/表单/FormField"\n参数：label（标签）；required（必填）；error（错误提示）；children（控件）\n规则：表单字段一律用它包裹，不要裸写 label + input。';
