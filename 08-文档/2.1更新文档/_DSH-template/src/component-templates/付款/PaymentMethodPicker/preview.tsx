...
import { PaymentMethodPicker, PAYMENT_METHOD_PRESETS } from './index';

const NAME = 'PaymentMethodPicker';
const DESC = '收银台、结算页、订阅扣款设置的支付渠道卡片单选（微信/支付宝/银行卡/余额）';
const PROMPT = '【组件引用】PaymentMethodPicker 支付方式选择（组件库 付款）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/付款/PaymentMethodPicker.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { PaymentMethodPicker } from \'../../component-templates/付款/PaymentMethodPicker\';\n参数：options/key/label/desc/icon/extra/tag/disabled；value/onChange/columns/showTitle\n场景：收银台、结算页、订阅扣款设置的支付渠道卡片单选（微信/支付宝/银行卡/余额）\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
...
function Demo() {
  ...
  const OPTIONS = PAYMENT_METHOD_PRESETS.map((o) =>
    o.key === 'wechat' ? { ...o, tag: '推荐' } : o.key === 'balance' ? { ...o, extra: '余额 ¥1,280.00' } : o
  );
  return (
    <DemoBoundary>
      <PaymentMethodPicker
