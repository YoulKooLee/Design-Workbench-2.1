...
import { InvoiceForm, InvoiceValue, InvoiceType } from './index';

const NAME = 'InvoiceForm';
const DESC = '收银台/订单详情的发票信息新增与编辑（个人/普票/专票，专票才要地址电话开户行账号）';
const PROMPT = '【组件引用】InvoiceForm 开票信息表单（组件库 付款）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/付款/InvoiceForm.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { InvoiceForm } from \'../../component-templates/付款/InvoiceForm\';\n参数：value{type,title,taxNo,address,phone,bank,bankAccount,email,remark}/onChange/errors/disabled/actions\n场景：收银台/订单详情的发票信息新增与编辑（个人/普票/专票，专票才要地址电话开户行账号）\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
...
function Demo() {
  const [value, setValue] = React.useState({
    type: 'vat',
    title: '青岛国实科技集团有限公司',
    taxNo: '91370212MA3D8N6X7Q',
    address: '青岛市崂山区松岭路 168 号',
    phone: '0532-88996611',
    bank: '中国银行青岛崂山支行',
    bankAccount: '2281001234567890',
    email: 'finance@guoshi-tech.cn',
    remark: ''
  });
  return (
    <DemoBoundary>
