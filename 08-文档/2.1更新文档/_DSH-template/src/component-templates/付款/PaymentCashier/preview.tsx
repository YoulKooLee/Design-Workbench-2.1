// @ts-nocheck
/**
 * 组件预览页 · 付款 / PaymentCashier
 * 由 03-组件库/03-页面组件/Codebuddy Design 的组件浏览页拆分而来（make「组件」页签 iframe 入口）。
 * 说明：预览页属文档性质，关闭类型检查；demo 拆自旧组件浏览页（部分依赖浏览页外部 state，点击交互时可能提示）。
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PaymentCashier } from './index';
import { PAYMENT_METHOD_PRESETS } from '../PaymentMethodPicker';

const NAME = 'PaymentCashier';
const DESC = '结算页与待支付订单：订单摘要+金额明细+支付方式+倒计时+支付按钮整块组装';
const PROMPT = '【组件引用】PaymentCashier 收银台（组件库 付款）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/付款/PaymentCashier.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { PaymentCashier } from \'../../component-templates/付款/PaymentCashier\';\n参数：orderNo/summaryItems/amountItems/totalAmount/methods/method/onMethodChange/deadline/onExpire/onPay/paying/notice\n场景：结算页与待支付订单：订单摘要+金额明细+支付方式+倒计时+支付按钮整块组装\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';

/** 兜底：默认参数无法渲染时给出提示，避免整页白屏 */
class DemoBoundary extends React.Component<{ children: React.ReactNode }, { err: string | null }> {
  state = { err: null as string | null };
  static getDerivedStateFromError(e: unknown) {
    return { err: e instanceof Error ? e.message : String(e) };
  }
  render() {
    if (this.state.err) {
      return (
        <div style={{ padding: 16, color: '#86909c', fontSize: 12, lineHeight: 1.7 }}>
          默认参数无法直接渲染该组件（{this.state.err}）。请参考下方提示词在原型页中引用。
        </div>
      );
    }
    return this.props.children;
  }
}

function Demo() {
  const [method, setMethod] = React.useState('wechat');
  const [paying, setPaying] = React.useState(false);
  const deadline = React.useMemo(() => Date.now() + 30 * 60 * 1000, []);
  return (
    <DemoBoundary>
      <PaymentCashier
        orderNo="SO20260922001"
        summaryItems={[
          { label: '商品名称', value: '水质分析仪探头等 3 件' },
          { label: '下单时间', value: '2026-09-22 09:12' },
          { label: '收货人', value: '张伟 138****6621' },
          { label: '收货地址', value: '山东省青岛市崂山区松岭路 168 号 A 座 12 层', span: 2 }
        ]}
        amountItems={[
          { label: '商品小计', value: 128600 },
          { label: '优惠券', value: -10000 },
          { label: '运费', value: 200 }
        ]}
        totalAmount={118800}
        methods={PAYMENT_METHOD_PRESETS}
        method={method}
        onMethodChange={(key) => setMethod(key)}
        deadline={deadline}
        paying={paying}
        onPay={() => { setPaying(true); setTimeout(() => setPaying(false), 1500); }}
        notice="该订单金额较大，建议使用企业对公账户支付。"
      />
    </DemoBoundary>
  );
}

function App() {
  const [copied, setCopied] = React.useState(false);
  const copy = async () => {
    try { await navigator.clipboard.writeText(PROMPT); } catch { /* 忽略剪贴板权限 */ }
    setCopied(true);
    setTimeout(() => setCopied(false), 1500);
  };
  return (
    <div style={{ padding: 24, maxWidth: 1180 }}>
      <h2 style={{ fontSize: 16, margin: '0 0 4px' }}>{NAME}</h2>
      <p style={{ fontSize: 12, color: '#86909c', margin: '0 0 14px' }}>{DESC}</p>
      <div style={{ border: '1px solid #e5e6eb', borderRadius: 8, background: '#fafbfc', padding: 20, marginBottom: 14 }}>
        <Demo />
      </div>
      <button onClick={copy} style={{ border: '1px solid #e5e6eb', background: '#fff', borderRadius: 6, padding: '5px 12px', fontSize: 12, cursor: 'pointer', color: copied ? '#00b42a' : '#4e5969' }}>
        {copied ? '已复制' : '复制提示词'}
      </button>
      <p style={{ fontSize: 11, color: '#a9aeb8', marginTop: 10, whiteSpace: 'pre-wrap' }}>{PROMPT}</p>
    </div>
  );
}

createRoot(document.getElementById('root')!).render(<App />);
