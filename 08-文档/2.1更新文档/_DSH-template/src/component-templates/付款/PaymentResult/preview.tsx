...
import { PaymentResult, PaymentResultState } from './index';
import { Button } from '../../基础/Button';

const NAME = 'PaymentResult';
const DESC = '支付回调结果页三态（成功/失败/处理中），可嵌弹窗';
const PROMPT = '【组件引用】PaymentResult 支付结果页（组件库 付款）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/付款/PaymentResult.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { PaymentResult } from \'../../component-templates/付款/PaymentResult\';\n参数：state success|fail|pending；title/desc/amount/orderNo/methodName/paidAt/tradeNo/reason/actions\n场景：支付回调结果页三态（成功/失败/处理中），可嵌弹窗\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
    ...
    <DemoBoundary>
      <div style={{ display: 'grid', gap: 16 }}>
        <PaymentResult
          ...
          methodName="微信支付"
          paidAt="2026-09-22 09:15:37"
          tradeNo="42000028932026092200018931"
          actions={<Button variant="primary">查看订单</Button>}
        />
        <PaymentResult
          state="fail"
          amount={118800}
          orderNo="SO20260922001"
          reason="银行卡余额不足，请更换支付方式后重试。"
          actions={<Button variant="outline">重新支付</Button>}
        />
      </div>
    </DemoBoundary>
  );
