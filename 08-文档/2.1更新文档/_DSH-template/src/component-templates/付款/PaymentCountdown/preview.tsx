...
import { PaymentCountdown } from './index';

const NAME = 'PaymentCountdown';
const DESC = '收银台「请在 xx 内完成支付」；归零触发 onExpire 关闭订单（真实计时器，勿手写 setInterval）';
const PROMPT = '【组件引用】PaymentCountdown 待支付倒计时（组件库 付款）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/付款/PaymentCountdown.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { PaymentCountdown } from \'../../component-templates/付款/PaymentCountdown\';\n参数：seconds/deadline/onExpire/format/size/showHint/hint/expiredText\n场景：收银台「请在 xx 内完成支付」；归零触发 onExpire 关闭订单（真实计时器，勿手写 setInterval）\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
  ...
  return (
    <DemoBoundary>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 32, alignItems: 'center' }}>
        <PaymentCountdown seconds={1799} size="lg" />
        <PaymentCountdown seconds={299} size="sm" showHint={false} />
      </div>
    </DemoBoundary>
  );
}
