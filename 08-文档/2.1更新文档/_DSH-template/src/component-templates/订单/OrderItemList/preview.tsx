...
import { OrderItemList } from './index';
import { Button } from '../../基础/Button';

const NAME = 'OrderItemList';
const DESC = '订单详情的商品清单、退款单可退商品列表（配 renderActions 做「申请退款」）';
const PROMPT = '【组件引用】OrderItemList 订单商品明细（组件库 订单）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/订单/OrderItemList.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { OrderItemList } from \'../../component-templates/订单/OrderItemList\';\n参数：data/loading?/title/showTitle/showSummary/amountHint/renderActions\n场景：订单详情的商品清单、退款单可退商品列表（配 renderActions 做「申请退款」）\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
...
function Demo() {
  const ITEMS = [
    { id: 1, name: '多参数水质分析仪探头', spec: '型号 WQ-200 / 5m 线缆', price: 32800, quantity: 2, sku: 'WQ200-5M' },
    { id: 2, name: '水下推进器', spec: 'T-500 / 无刷 / 含驱动器', price: 18600, quantity: 1, sku: 'THR-500' },
    { id: 3, name: '耐压舱密封圈套装', spec: 'DN100 / 含 8 件', price: 1900, quantity: 3, sku: 'SEAL-DN100' }
  ];
  return (
    <DemoBoundary>
      <OrderItemList
        data={ITEMS}
        amountHint="元（含税，不含运费）"
        renderActions={() => <Button variant="text" size="sm">申请退款</Button>}
      />
    </DemoBoundary>
  );
}
