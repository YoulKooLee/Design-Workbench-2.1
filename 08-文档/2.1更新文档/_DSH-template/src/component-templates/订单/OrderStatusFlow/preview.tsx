...
import { OrderStatusFlow, ORDER_FLOW_STEPS } from './index';
import { Button } from '../../基础/Button';

const NAME = 'OrderStatusFlow';
const DESC = '订单详情履约进度、退款单/工单阶段展示；closed 表终止、error 表异常';
const PROMPT = '【组件引用】OrderStatusFlow 订单状态流转（组件库 订单）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/订单/OrderStatusFlow.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { OrderStatusFlow } from \'../../component-templates/订单/OrderStatusFlow\';\n参数：current/steps/error/closed/closedReason/direction/extra/showTitle\n场景：订单详情履约进度、退款单/工单阶段展示；closed 表终止、error 表异常\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
  ...
  return (
    <DemoBoundary>
      <div style={{ display: 'grid', gap: 16 }}>
        <OrderStatusFlow
          current={2}
          extra={<Button size="sm" variant="outline">提醒发货</Button>}
        />
        <OrderStatusFlow
          current={0}
          closed
          closedReason="买家在 30 分钟内未完成支付，订单已自动关闭"
        />
      </div>
    </DemoBoundary>
  );
