...
import { TriggerTypePicker, TRIGGER_TYPE_PRESETS } from './index';

const NAME = 'TriggerTypePicker';
const DESC = '自动化规则/触发器/工作流第一步「何时触发」（事件/定时/Webhook/手动）';
const PROMPT = '【组件引用】TriggerTypePicker 触发方式选择（组件库 触发器）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/触发器/TriggerTypePicker.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { TriggerTypePicker } from \'../../component-templates/触发器/TriggerTypePicker\';\n参数：options/key/label/desc/icon/tag/disabled；value/onChange/columns/showTitle\n场景：自动化规则/触发器/工作流第一步「何时触发」（事件/定时/Webhook/手动）\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
...
function Demo() {
  ...
  const OPTIONS = TRIGGER_TYPE_PRESETS.map((o) =>
    o.key === 'schedule' ? { ...o, tag: '高频' } : o.key === 'webhook' ? { ...o, tag: '需鉴权' } : o
  );
  return (
    <DemoBoundary>
      <TriggerTypePicker
