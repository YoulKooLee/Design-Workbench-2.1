...
import { TriggerActionList } from './index';

const NAME = 'TriggerActionList';
const DESC = '自动化规则「满足条件后执行什么」的有序动作序列（上移/下移/删除，顺序即执行顺序）';
const PROMPT = '【组件引用】TriggerActionList 触发动作列表（组件库 触发器）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/触发器/TriggerActionList.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { TriggerActionList } from \'../../component-templates/触发器/TriggerActionList\';\n参数：actions[{id,type,label,summary,invalid}]/onChange/onAdd/onEdit/max/disabled\n场景：自动化规则「满足条件后执行什么」的有序动作序列（上移/下移/删除，顺序即执行顺序）\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
...
function Demo() {
  const [actions, setActions] = React.useState([
    { id: 1, type: 'email', label: '发送邮件通知', summary: '发送至 ops@ls-lab.cn、finance@ls-lab.cn' },
    { id: 2, type: 'webhook', label: '调用 Webhook', summary: 'POST https://api.ls-lab.cn/workflow/approve' },
    { id: 3, type: 'update', label: '更新订单字段', summary: '将 status 置为「已确认」', invalid: true }
  ]);
  return (
    <DemoBoundary>
      <TriggerActionList
        actions={actions}
        onChange={(next) => setActions(next)}
        onAdd={() => setActions((prev) => [...prev, { id: Date.now(), type: 'notify', label: '发送站内通知', summary: '通知申请人' }])}
      />
    </DemoBoundary>
  );
}
