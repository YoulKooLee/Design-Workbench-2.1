...
import { ContentStatusBadge, CONTENT_STATUS_PRESETS } from './index';

const NAME = 'ContentStatusBadge';
const DESC = '内容列表状态列、内容详情状态字段（草稿/待审/已发布/已下线/已驳回）';
const PROMPT = '【组件引用】ContentStatusBadge 内容状态标签（组件库 CMS）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/CMS/ContentStatusBadge.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { ContentStatusBadge } from \'../../component-templates/CMS/ContentStatusBadge\';\n参数：value/variant full|simple/map/className\n场景：内容列表状态列、内容详情状态字段（草稿/待审/已发布/已下线/已驳回）\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
...
function Demo() {
  const ALL = ['draft', 'reviewing', 'approved', 'scheduled', 'published', 'offline', 'rejected'];
  return (
    <DemoBoundary>
      <div style={{ display: 'grid', gap: 14 }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
          {ALL.map((v) => <ContentStatusBadge key={v} value={v} />)}
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
          {ALL.map((v) => <ContentStatusBadge key={v} value={v} variant="simple" />)}
        </div>
      </div>
    </DemoBoundary>
  );
