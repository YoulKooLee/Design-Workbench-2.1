// @ts-nocheck
/**
 * 组件预览页 · CMS / PublishSchedulePanel
...
import { PublishSchedulePanel, PublishScheduleValue, PublishScope } from './index';

const NAME = 'PublishSchedulePanel';
const DESC = 'CMS 内容发布前设置（立即/定时发布 + 发布范围 + 发布端 + 备注）';
const PROMPT = '【组件引用】PublishSchedulePanel 发布设置（组件库 CMS）\n框架：React 18 + Tailwind v4（零第三方依赖）\n来源：03-组件库/03-页面组件/Codebuddy Design/CMS/PublishSchedulePanel.tsx\n用法：先把「Codebuddy Design」整目录复制到工程 src/component-templates/axhub/，然后\n  import { PublishSchedulePanel } from \'../../component-templates/CMS/PublishSchedulePanel\';\n参数：value{mode,publishDate,publishTime,scope,categories,channels,remark}/onChange/categoryOptions/channelOptions/errors\n场景：CMS 内容发布前设置（立即/定时发布 + 发布范围 + 发布端 + 备注）\n规则：请使用该组件，不要自造同类样式；零第三方依赖。';
  ...
  return (
    <DemoBoundary>
      <PublishSchedulePanel
        ...
        notice="定时发布前请确认内容已通过审核，发布后可在内容列表撤回。"
      />
    </DemoBoundary>
  );
}
