// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageListFilterTable
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageListFilterTable, MOCK_CONTRACT_ROWS } from './index';

const NAME = 'PageListFilterTable';
const DESC = '左筛右表（9 项筛选 + 合同表 + 批量操作 + 导入导出）';
const PROMPT = '【组件引用】PageListFilterTable 左筛右表（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageListFilterTable } from "../../component-templates/页面模式/PageListFilterTable"\n参数：rows；pageSize\n场景：需要常驻筛选面板的业务列表（合同/订单/工单），含批量状态变更与导入导出。页面级组件，直接整页使用。';
