// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageDetailBasic
 * 页面级组件：直接整页渲染（自带标题与分区），下方附引用提示词。
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageDetailBasic, MOCK_DETAIL_BASIC_INFO } from './index';

const NAME = 'PageDetailBasic';
const DESC = '基础详情页（页头 + 5 张 Descriptions 卡，47 字段位）';
const PROMPT = '【组件引用】PageDetailBasic 基础详情页（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageDetailBasic } from "../../component-templates/页面模式/PageDetailBasic"\n参数：info（不传用内置演示数据）；extra；onEdit；onExport；onBack\n场景：内容/资源详情查看页（概览 + 视频/音频/适配参数）。页面级组件，直接整页使用。';
