// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageListCalendar
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageListCalendar, MOCK_CALENDAR_EVENTS } from './index';

const NAME = 'PageListCalendar';
const DESC = '日历页面（日历/列表双视图 + 事项抽屉与新建弹窗）';
const PROMPT = '【组件引用】PageListCalendar 日历页面（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageListCalendar } from "../../component-templates/页面模式/PageListCalendar"\n参数：events；defaultView\n场景：日程/会议/活动管理（月历视图 + 列表视图 + 新建与详情）。页面级组件，直接整页使用。';
