// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageListImport
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageListImport, MOCK_IMPORT_VALIDATE_ROWS } from './index';

const NAME = 'PageListImport';
const DESC = '导入向导（上传 / 数据校验 / 结果三步）';
const PROMPT = '【组件引用】PageListImport 导入向导（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageListImport } from "../../component-templates/页面模式/PageListImport"\n参数：defaultStep；validateRows；onDownloadTemplate；onImported；onBackToList\n场景：批量导入（拖拽上传 + 校验明细 + 结果统计）。页面级组件，直接整页使用。';
