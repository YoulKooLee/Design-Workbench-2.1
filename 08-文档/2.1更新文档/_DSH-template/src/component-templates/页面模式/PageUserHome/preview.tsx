// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageUserHome
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageUserHome, MOCK_USER_PROJECTS, MOCK_USER_ACTIVITIES, MOCK_USER_TEAMS, MOCK_USER_NOTICES } from './index';

const NAME = 'PageUserHome';
const DESC = '个人主页（头像 + 项目 + 动态 + 团队 + 通知）';
const PROMPT = '【组件引用】PageUserHome 个人主页（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageUserHome } from "../../component-templates/页面模式/PageUserHome"\n参数：userName；profile；projects；activities；teams；notices；onMore\n场景：用户个人中心首页（项目卡片、动态流、团队成员、站内通知）。页面级组件，直接整页使用。';
