// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageListSearchTable
...
import { PageListSearchTable } from './index';

const NAME = 'PageListSearchTable';
const DESC = '筛选 + 表格 + 分页 + 详情抽屉 + 新建编辑 + 导入导出 的完整列表页';
const PROMPT = '【组件引用】PageListSearchTable 查询表格页（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageListSearchTable } from "../../component-templates/页面模式/PageListSearchTable"\n参数：title；desc；dataSource（不传用内置 68 条 mock）；defaultPageSize；onSave；onDelete\n场景：CRUD 列表页整合（含 8 项筛选、行选择、列设置、密度、详情抽屉、导入导出）。页面级组件，直接整页使用，不要拆散重写。';
