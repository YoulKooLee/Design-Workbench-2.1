// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageFormStepVertical
 * 页面级组件：直接整页渲染（自带标题与分区），下方附引用提示词。
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageFormStepVertical } from './index';

const NAME = 'PageFormStepVertical';
const DESC = '纵向分步表单（4 步纵向轨道 + 30 字段 + 成功页信息汇总）';
const PROMPT = '【组件引用】PageFormStepVertical 纵向分步表单页（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageFormStepVertical } from "../../component-templates/页面模式/PageFormStepVertical"\n参数：defaultStep（1-4）；defaultValues；onSubmit；onBackHome\n场景：字段多、需边填边对照进度的表单（纵向步骤轨道）。页面级组件，直接整页使用。';
