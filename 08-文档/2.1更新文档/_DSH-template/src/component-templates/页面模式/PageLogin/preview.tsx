...
const DESC = '登录页（banner / cover / split 三形态 + 找回与注册）';
const PROMPT = '【组件引用】PageLogin 登录页（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageLogin } from "../../component-templates/页面模式/PageLogin"\n参数：variant（banner|cover|split）；appName；subTitle；defaultUserName；defaultPassword；onLogin；onForgot；onRegister\n场景：账号登录（演示账号 admin / 123456），含记住密码、忘记密码（邮箱+验证码）、注册账号。页面级组件，直接整页使用。';
