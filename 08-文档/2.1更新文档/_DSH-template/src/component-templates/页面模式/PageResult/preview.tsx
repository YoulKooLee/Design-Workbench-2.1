...
const NAME = 'PageResult';
const DESC = '结果页（成功 / 等待 / 警告 / 失败 + 差异化操作）';
const PROMPT = '【组件引用】PageResult 结果页（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageResult } from "../../component-templates/页面模式/PageResult"\n参数：type；title；desc；children；onBack；onViewList；onRefresh；onAcknowledge；onRetry\n场景：表单提交/异步任务的结果反馈页（成功→查看列表、等待→刷新、警告→确认、失败→重试）。页面级组件，直接整页使用。';
