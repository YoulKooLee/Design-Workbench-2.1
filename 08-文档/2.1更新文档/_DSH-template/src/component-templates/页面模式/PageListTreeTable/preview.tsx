// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageListTreeTable
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageListTreeTable, MOCK_EMPLOYEE_ROWS, MOCK_DEPARTMENT_TREE } from './index';

const NAME = 'PageListTreeTable';
const DESC = '左树右表（部门树 + 成员表 + 批量停用 + 详情抽屉）';
const PROMPT = '【组件引用】PageListTreeTable 左树右表（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageListTreeTable } from "../../component-templates/页面模式/PageListTreeTable"\n参数：rows；pageSize\n场景：按组织架构（部门树）筛选的人员/资源列表。页面级组件，直接整页使用。';
