// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageFormStep
 * 页面级组件：直接整页渲染（自带标题与分区），下方附引用提示词。
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageFormStep } from './index';

const NAME = 'PageFormStep';
const DESC = '分步表单（3 步步骤条 + 分步校验 + ResultPage 成功页）';
const PROMPT = '【组件引用】PageFormStep 分步表单页（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageFormStep } from "../../component-templates/页面模式/PageFormStep"\n参数：defaultStep（1|2|3）；defaultValues；onSubmit；onBackHome；onViewList\n场景：多步骤表单（3 步）。页面级组件，直接整页使用。';
