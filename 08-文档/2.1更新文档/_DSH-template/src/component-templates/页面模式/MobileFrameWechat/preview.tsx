...
import { MobileFrameWechat } from './index';

const NAME = 'MobileFrameWechat';
const DESC = '微信小程序壳（胶囊 + 四形态）';
const PROMPT = '【组件引用】MobileFrameWechat 微信小程序壳（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { MobileFrameWechat } from "../../component-templates/页面模式/MobileFrameWechat"\n参数：variant（plain|header|nav|tabs）；title；navItems；tabs；width；children\n场景：把小程序页面装进微信外框做演示（含胶囊按钮与底部标签栏）。页面级组件，直接整页使用。';
...
function App() {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1400, margin: '0 auto' }}>
      ...
      <DemoBoundary><MobileFrameWechat /></DemoBoundary>
