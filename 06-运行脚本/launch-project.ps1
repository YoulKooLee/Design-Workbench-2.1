...
function Get-ProjectVitePort($projDir) {
    ...
    foreach ($proc in $procs) {
        ...
        $vports = @()
        $vl = & netstat -ano 2>$null
        foreach ($_l in $vl) {
            if ($_l -match '^\s*TCP\s+\S+:(\d+)\s+\S+\s+LISTENING\s+(\d+)\s*$') {
                if ([int]$Matches[2] -eq $proc.ProcessId -and [int]$Matches[1] -ge 51700 -and [int]$Matches[1] -le 51800) { $vports += [int]$Matches[1] }
            }
        }
        if ($vports.Count -gt 0) { return [int]$vports[0] }
    }
    return 0
}
