...
在已复制 vibepm-dev-agent（`.agents/` + `AGENTS.md`）的前提下，按本文创建前端子项目并接入 `feature-dev`。
...
|------|------|----------------|--------------|
| 单项目 | 根目录即前端 | 项目根 | 项目根 |
| 多子项目 | `admin/`、`element-plus/`、`ant-design-pro/`、`shadcn/`、`mobile/` 等并列 | 仓库根 | 各子目录 |
...
## 第 6 步：准备 docs（若要写 SRS / 用 feature-dev）
...
cd {PROJECT_PATH}
pnpm dev          # 能启动
pnpm build        # 能构建（feature-dev 收尾会执行）
```

## 第 8 步：Hook 接线（可选）

非必须。未接线时按 `.agents/rules/soft-gates.md` 用软门禁执行。

若当前 ADE 支持生命周期 Hook，按 [adapters/README.md](../.agents/adapters/README.md) 复制薄接线：Claude Code → `.claude/settings.json`；Cursor → `.cursor/hooks.json`。不要把整套 skills 复制进这些目录。

Cline / Roo / Copilot 见 [adapters/other-ade.md](../.agents/adapters/other-ade.md)：Roo 与 Copilot 可直接读 `.agents/skills/`；Cline 用联接脚本，禁止拷贝。

## 第 9 步：开始 AI 生成页面

1. 无确认设计方案时先 `brainstorming`，再 `srs-writer` 写 SRS（或使用已有 SRS）
2. 触发 `feature-dev`：「按照 SRS 实现 xxx 功能」
3. AI 将自动：
   - 选定 `PROJECT_PATH`
