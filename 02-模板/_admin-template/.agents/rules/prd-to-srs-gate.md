...
| **仅有 PRD、无 SRS** | **阻断**研发类技能，输出 §3 标准话术，路由 **`srs-writer` Step F** |
| **SRS + PRD 并存** | 以 **SRS** 为 `SPEC_SOURCE`；SRS 文首应链回 PRD；不阻断 |
| **皆无** | 按场景：`srs-writer` 生成 或 `prd-writer` 探索 |

**结构完整性快检**（SRS）：文件含 `3.1`/`3.3`/`3.5` 标题或等价编号；缺任一则视为「不完整 SRS」，同「仅有 PRD」处理或走 `srs-writer` 局部完善。
...
feature-dev / 交付计划 / 概要设计 等技能需要 SRS 章节结构（3.1 / 3.3 / 3.5 / 6.1），不能直接用 PRD（§4 / §5）实现。

建议下一步（任选其一）：
  1. 【推荐】转写 SRS：说「PRD 转 SRS」或「进开发」→ 执行 srs-writer Step F
  2. 若尚未定稿 PRD → 先完成 prd-writer 自检后再转写
  3. 【降级】快速原型：明确说「跳过 SRS，按 PRD 手动对齐实现」→ 仅本次 feature-dev 放行，不登记为正式交付
...
仅当用户消息 **明确包含** 以下之一时，允许 `feature-dev` 在无 SRS 下继续（**单次会话**）：
...
4. 完成后提示：「正式交付前请执行 srs-writer Step F 补 SRS」
...
`srs-writer` Step F 完成后须满足以下 **全部** 项，方可解除门禁：
...
来源 PRD：{PRD路径}
SRS 真源：{SRS路径}
SPEC_SOURCE 已更新 → 可继续 feature-dev / delivery-plan / hld-design
```
...
| 文件 | 用途 |
| --- | --- |
| `.agents/skills/srs-writer/SKILL.md` Step F | 转写执行流程 |
| `.agents/skills/srs-writer/references/prd-to-srs-handoff.md` | PRD 章节 → SRS 章节映射与展开规则 |
