...
没有规范扫描的需求修改 = 违规
没有跨章节同步检查的局部修改 = 违规
从零新建 SRS 且无确认设计方案、四项信息未齐 = 先 brainstorming
```

## 适用范围

对 SRS 需求规格说明书（`*需求*说明书*.md`）的任何修改操作，包括：
- 通过 srs-writer 技能的完整流程
- 主流程直接 Edit 修改
- 用户要求的局部调整

无论修改大小，无论是否调用技能，以下流程都是强制的。

**从零新建** `*需求说明书*` 时：仓库没有确认过的设计方案，且用户未一次给齐项目名称、背景和目标、核心功能模块、目标用户 → 先走 `brainstorming`，禁止直接 Write SRS。已有 SRS 的修改（完善/审查/反向同步）不走本条；PRD→SRS 走 `srs-writer` Step F。
...
| **用户手动 Edit SRS、未走 srs-writer** | 完整扫描（门禁 2–4 全部执行） |
