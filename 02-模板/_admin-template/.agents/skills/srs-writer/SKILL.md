...
用于生成、撰写、创建、细化、审查或反向同步 SRS 需求规格说明书的技能。
  旧名 req-doc：用户说 req-doc、/req-doc 时仍走本技能。
触发场景：(1) "生成需求说明书" "写需求说明书" "创建需求文档" "需求文档" "SRS" "SRS需求" "需求规格说明书" "req-doc",
...
(10) "PRD转SRS" "PRD 转 SRS" "进开发" "转写需求" "按 PRD 写 SRS" "PRD 转需求说明书" 或仅有 *-PRD.md 却要 feature-dev / 交付计划 / 概要设计时。
...
本技能适用：文档类；宣称完成可省略 build，用结构检查与禁用词 grep 作证据；`req-reviewer` 按已有 A6 三档（**快速整步跳过**）。
禁止因无 Hook 把快速模式升为全量审查。

## 角色调度架构

```
srs-writer skill（主流程：调度 + 文件写入）
  ├── req-analyzer 角色（分析师）
...
| 模式 | 触发场景 | 入口 |
| --- | --- | --- |
| **生成模式** | 从零生成SRS需求规格说明书 | → Step A（先过 **A1.5**；B–F 不过此门禁） |
...
> **门禁**：研发类技能（`feature-dev` 等）无 SRS 时须先 Step F。规则见 `.agents/rules/prd-to-srs-gate.md`。
...
A1 扫完后立刻判定，**通过才能进入 A2**。Step B/C/D/E/F **不适用**本门禁。

**足够（任一即可）→ 继续 A2：**

- 已有 `*设计方案*.md`，且用户本轮确认「按这份写」或未提出推翻
- 用户一次给齐四项：项目名称、背景和目标、核心功能模块列表、目标用户群体
- 已有 SRS，实际是增量 → 转 **Step C**
- 已有 `*-PRD.md`，实际是转写 → 转 **Step F**

**不足 → 停，不读模板、不出 A4 清单：**

- 只有一句话想法或短清单，且没有确认过的设计方案
- 模块列表互相矛盾或边界说不清

固定话术（只说一次）：

> 当前还没有确认过的设计方案，先用头脑风暴对齐范围和方案。确认落盘后，再回来生成 SRS（不会把刚才问过的再问一遍）。

然后 Read `.agents/skills/brainstorming/SKILL.md` 并执行；**本技能暂停**。头脑风暴结束且用户选 SRS 后，从本门禁的「足够」分支继续（A3 只补缺口）。
...
四项必填：项目名称、项目背景和目标、核心功能模块列表、目标用户群体。**有来源则抽取，只问缺失或互相打架的项。**

| 已有来源 | A3 动作 |
| --- | --- |
| 已 Read 设计方案 | 从文档抽取四项；**不重问**目的/用户/模块 |
| 用户本轮已给齐四项 | 复述清单请用户确认，**不再逐项追问** |
| 同一会话刚从 `brainstorming` 接回 | 直接用设计方案；登记 `SOURCE_PATH` |
| 四项仍缺 | 只问缺的项；若实质仍是「一个想法」→ 回 **A1.5** |

**按需询问（来源未写明时才问）：** 系统类型（Web/移动端/多端）、特殊业务规则

**术语表冷启动检查：**

检查 `.agents/agent-memory/req-writer/` 目录是否存在项目术语表文件：
- 若存在 → 读取并在 A4 任务清单中注明"已有术语表，analyzer 将复用"
- 若不存在 → 在任务清单第一个 [角色] 任务备注"首次运行，analyzer 将初始化术语表"，并在第一组 analyzer 全部完成后，主流程汇总各 analyzer 输出的术语，写入 `.agents/agent-memory/req-writer/project-{项目名}-terminology.md`，后续各组 analyzer 直接读取复用
...
**[角色] 任务分组规则：每 2-3 个功能模块为一组，同组并行 analyzer → 同组并行 writer → 串行写入。**
...
| 9  | 3.5.1 首页 | [角色] | 第1组 | [ ] 等待中 |
| 10 | 3.5.2 数据看板 | [角色] | 第1组 | [ ] 等待中 |
| 11 | 3.5.3.1 基础档案 | [角色] | 第1组 | [ ] 等待中 |
| 12 | 生成供应商管理+用户管理+任务计划 操作流程图+时序图 | [图表] | 第2组 | [ ] 等待中 |
| 13 | 3.5.3.2 供应商管理 | [角色] | 第2组 | [ ] 等待中 |
| 14 | 3.5.4 用户管理 | [角色] | 第2组 | [ ] 等待中 |
| 15 | 3.5.5 任务计划 | [角色] | 第2组 | [ ] 等待中 |
...
```

**关键规则：**
- 每个 [角色] 任务之前必须有对应的 [图表] 任务，同组的图表任务合并为一个批次
- [图表] 任务完成后将图片路径记录在备注列，[角色] 任务执行时从备注列读取路径传给 req-writer
- 同组 [角色] 任务的 analyzer 阶段并行，writer 阶段并行，文件写入按编号顺序串行
...
- [角色]：调度 analyzer → writer → reviewer 流程生成（所有 3.5.x 功能详细设计）

### A5: 执行任务

**执行规则：**
1. [文档] 和 [图表] 类任务：每次只执行一个，执行前更新为 [进行中]，完成后更新为 [完成]
2. [角色] 类任务：按分组执行，同组内并行 analyzer → 并行 writer → 串行写入；无独立派发时改为组内串行扮演，仍不得跳过 writer/reviewer 范围
...
同组的图表任务合并为一个批次调用 `diagram-generator` 技能，获取所有图片路径后记录在任务清单备注中，供后续 [角色] 任务使用。

**[角色] 类任务执行方式（分组并行）：**

每组功能模块按 `role-dispatch.md` 调度。无独立派发时组内串行扮演。流程：

```
Phase 1 — 并行 analyzer（同组所有模块同时启动；无独立派发则串行扮演）
  对组内每个模块调度 req-analyzer：
【角色】req-analyzer
【定义】.agents/agents/req-analyzer.md
【交付模式】{DELIVERY_MODE}
...
交付模式：{DELIVERY_MODE}
      PROJECT_PATH: {项目根目录}
      FEATURE_NAME: {功能名称}
      MODE: 有设计方案时 from_design；仅有代码时 from_code
      SOURCE_PATH: {已确认设计方案路径优先；否则对应 src/ 目录}
【输出】四状态：DONE / DONE_WITH_CONCERNS / NEEDS_CONTEXT / BLOCKED
  等待组内所有 analyzer 完成后进入 Phase 2

Phase 2 — 并行 writer（同组所有模块同时启动；无独立派发则串行扮演）
  对组内每个模块调度 req-writer：
【角色】req-writer
【定义】.agents/agents/req-writer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。不得因环境能力不足缩小范围。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
...
**注意：** diagram-generator 是技能（Skill），不是角色，无法在角色协作流程中间调用。流程图必须作为独立的 [图表] 类任务，在对应 [角色] 任务之前完成，再将图片路径传给 req-writer。
...
| 模式 | 动作 |
| --- | --- |
| **快速** | 跳过 req-reviewer；对本次新增/变更段落做 **禁用词 grep**（见 `srs-writer-workflow.md`） |
| **标准** | 对 3.5.x **抽检**：`max(2, ceil(模块数 × 20%))` 个模块并行 req-reviewer；优先抽 **首个、中间、末尾** 模块 |
| **严格** | **全部** 3.5.x 按 2–3 个一组并行 req-reviewer |

```
【角色】req-reviewer
【定义】.agents/agents/req-reviewer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。快速模式本步整步跳过，不扮演、不派发。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
  PROJECT_PATH: {项目根目录}
  FEATURE_NAME: {功能名称}
...
对 B2 中标记为"需要更新"或"需要新增"的 3.5.x 功能模块，按 2-3 个一组并行调度 `req-analyzer`（无独立派发则串行扮演）：
...
| 4  | 三、3.5.1 首页 | [角色] | [ ] 等待中 |
| 5  | 三、3.5.2 数据看板 | [角色] | [ ] 等待中 |
| 6  | 三、3.5.3.1 基础档案 | [角色] | [ ] 等待中 |
...
```
...
- **结构性章节**：主流程对照模板检查
- **3.5.x**：按交付模式 — **严格** 全量 req-reviewer；**标准** 仅审查 D2 清单中标记 `[角色]` 的章节（用户点名审查时可全量）；**快速** 跳过 reviewer，grep 禁用词
...
**若两者都不可用，降级到解压 XML：**
```bash
unzip -p {docx路径} word/document.xml > /tmp/srs-writer-raw.xml
# 提示用户安装 docling 以获得更好的转换质量
```

转换完成后，读取 `/tmp/req-template-source.md` 的内容。

### E3: 调度 template-analyzer

```
【角色】template-analyzer
【定义】.agents/agents/template-analyzer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
...
```
{PROJECT_PATH}/.agents/skills/srs-writer/references/templates/{模板文件名}.md
```
...
```bash
ls {PROJECT_PATH}/.agents/skills/srs-writer/references/templates/*.md
```
...
下一步：/delivery-plan 生成交付计划，或 /feature-dev 实现{首个模块}
```
...
```powershell
.agents/skills/common/export-word.ps1 <markdown文件路径> srs-writer
```

**Git Bash / Linux / macOS / Python：**

```bash
python .agents/skills/common/export-word.py <markdown文件路径> srs-writer
# 或
bash .agents/skills/common/export-word.sh <markdown文件路径> srs-writer
```

> 更多模板见 `.agents/skills/common/README.md`。Word API 模板键仍为 `req-doc`；传入 `srs-writer` 会自动映射。

## 文档命名规范

`docs/01-需求与规划/{日期}-{客户名称}{项目名称}-SRS需求规格说明书-V{版本号}.md`

示例：`docs/01-需求与规划/20260818-示例科技客户关系管理模块-SRS需求规格说明书-V1.0.md`
