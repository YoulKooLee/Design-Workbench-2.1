...
name: brainstorming
description: >
  在实现功能或落需求文档之前，先探索用户意图并产出设计方案。
  触发：用户描述想构建什么、分享功能想法、提供简要需求清单、要求对齐方案或写设计方案；创建功能/组件且方案未对齐时。
  不触发：已点名 SRS/需求说明书/审查/反向同步/PRD转SRS；或已有确认设计方案且要求直接写说明书。
---

# 头脑风暴：从想法到设计方案

通过自然对话将用户的想法或需求清单转化为完整的设计方案，确认后保存文档，再询问 SRS 还是 PRD。选 SRS 时回传 `srs-writer`，不再重复问诊。
...
```
理解输入 → 澄清问题（逐个） → 提出方案 → 展示设计 → 用户确认 → 保存文档 → 询问 SRS 或 PRD
```
...
- 示例：`docs/01-需求与规划/2026-08-18-示例科技客户关系管理模块-设计方案-v1.0.md`
- 同名文件已存在时自动递增版本号（v1.1, v1.2...）
- 与 srs-writer 技能的命名风格保持一致（日期-客户名称项目名称-文档类型-版本号）
...
> 设计文档已保存。接下来：**SRS（srs-writer）**、**PRD（prd-writer）**，或暂不需要？  
> 也可说「快速模式 + PRD」直接出稿。  
> **若选 PRD 且后续要开发** → PRD 确认后须 **`srs-writer` Step F** 转 SRS，再 `feature-dev`（见 `.agents/rules/prd-to-srs-gate.md`）。

会话登记：`DESIGN_SOURCE={设计方案路径}`，供下游当 `SOURCE_PATH`。

- **SRS** → Read `.agents/skills/srs-writer/SKILL.md`，从 **A1.5 足够分支** 进入；A3 **只补缺口**，禁止把目的/用户/模块再问一遍
