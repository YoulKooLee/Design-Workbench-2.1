...
本技能适用：文档类；宣称完成可省略 build；`design-reviewer` 按本技能已有调度。
...
- **仅有 PRD、无 SRS** → Read `.agents/rules/prd-to-srs-gate.md`，**中止**，路由 **`srs-writer` Step F**
...
模块详细设计（三、）按模块拆分为 [角色] 任务并分组；数据库（四、）和 API（五、）有两种编排：
...
| 3 | 三、3.1 模块A 详细设计 | [角色] | 第1组 | [ ] 等待中 |
| 4 | 三、3.2 模块B 详细设计 | [角色] | 第1组 | [ ] 等待中 |
| 5 | 生成 模块C 类图+时序图 | [图表] | 第2组 | [ ] 等待中 |
| 6 | 三、3.3 模块C 详细设计 | [角色] | 第2组 | [ ] 等待中 |
| 7 | 生成 物理ER图 | [图表] | - | [ ] 等待中 |
| 8 | 四、数据库物理设计（汇总所有表） | [角色] | - | [ ] 等待中 |
| 9 | 五、API 详细设计（汇总所有接口） | [角色] | - | [ ] 等待中 |
| 10 | 六、安全设计 + 七、可追溯矩阵 | [角色] | - | [ ] 等待中 |
```

### A5: 执行任务

**[角色] 类任务分组执行：** 无独立派发时组内串行扮演，审查范围不变。

```
Phase 1 — 并行 analyzer（组内模块同时启动；无独立派发则串行扮演）
【角色】design-analyzer
【定义】.agents/agents/design-analyzer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
...
Phase 2 — 并行 writer（组内模块同时启动；无独立派发则串行扮演）
【角色】design-writer
【定义】.agents/agents/design-writer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
...
1. **并行审查**：模块章节、数据库章节、API章节分别按 `role-dispatch.md` 调度 design-reviewer（DOC_TYPE=lld；无独立派发则串行扮演）。传入「章节内容 + 对应摘要」——模块章节传该模块在 A5 按模块名索引保存的 analyzer 摘要；数据库/API 汇总章节传 A5 汇总去重后的表/接口摘要。数据库和API章节是审查重点（必备字段、命名、索引、统一响应格式）。

```
【角色】design-reviewer
【定义】.agents/agents/design-reviewer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。不得因环境能力不足缩小审查范围。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
DOC_TYPE: lld
章节内容: {对应章节}
...
1. **分析代码现状**：以路由注册表为唯一真相源——只有实际注册的路由才算"功能存在"，残留文件不算（判定细则见 `.agents/skills/srs-writer/SKILL.md` 的 Step B）。对每个活跃模块，design-analyzer（from_code）提取：实际表字段（mock/entity）、实际接口（api/*.ts）、实际类结构
...
示例：`docs/02-架构与设计/20260818-示例科技客户关系管理模块-详细设计说明书-V1.0.md`
