...
本技能适用：文档类；宣称完成可省略 build；`feasibility-reviewer` 按本技能已有调度，无跳过表则不新造跳过。
...
### A3：调度 feasibility-analyzer 分析项目文档

按 `.agents/rules/role-dispatch.md` 调度：

```
【角色】feasibility-analyzer
【定义】.agents/agents/feasibility-analyzer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
...
**任务2-7（角色流程）：** 每个章节按以下步骤执行：

```
Step 1: 调度 feasibility-writer
【角色】feasibility-writer
【定义】.agents/agents/feasibility-writer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
...
Step 2: 调度 feasibility-reviewer 审查
【角色】feasibility-reviewer
【定义】.agents/agents/feasibility-reviewer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
...
- 导出失败：检查 `.agents/skills/common/config.json` 中的 `apiBaseUrl` 是否配置正确
...
示例：`docs/01-需求与规划/2026-08-18-示例科技客户关系管理模块-可行性研究报告-V1.0.md`
