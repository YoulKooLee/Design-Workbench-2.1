...
// 编辑器保存（须 await：保存失败时勿关闭编辑器并给出提示）
const handleEditorSave = async (data: { title: string; content: string; category: AnnotationCategory; source: string; color: string }) => {
  try {
    if (editorItem.value) {
      await updateAnnotation(editorItem.value.id, data)
    } else {
      ...
      await addAnnotation(newItem)
    }
    editorVisible.value = false
  } catch (e) {
    const msg = e instanceof Error ? e.message : '标注保存失败'
    window.alert(`${msg}\n\n请确认已用 npm run dev 启动，并已重启过开发服务。`)
  }
}
