...
本技能适用：改源码时须 verification，并按 `AGENTS.md` 勾选 `code-reviewer`；步骤 5 的 JSON 校验保留，但不能单独充当改了 Vue/TS 后的完成证据。不新造快速跳过表。
...
**门禁**：若未找到 SRS 但存在 `docs/**/*-PRD.md` → Read `.agents/rules/prd-to-srs-gate.md`，**中止**，路由 **`srs-writer` Step F**。
...
3. **注册标注保存中间件（Vite 项目必做，每次都检查 —— 缺失则标注只能显示、无法保存）**：
  ...
  - **铁律（曾导致「编辑确定后 JSON 不更新 / 刷新丢失」）**：插件须在 `configureServer` 内**同步** `server.middlewares.use(...)`，并设 `enforce: 'pre'`。**禁止** `return () => { middlewares.use(...) }`（Vite post hook）。post hook 排在内部中间件之后，未知 `POST` 可能已被 404，表现为 `/__annotation_save__` 变 404、标注只留在内存。
  - 注册/覆盖插件后须**重启** `npm run dev` 才生效。
  - 若项目里已有旧的 `vite-plugins/annotation-dev-save.ts` 等重复保存插件，保留技能版 `annotationPlugin()` 即可，避免双注册。
  - **原因**：编辑模式下新增/修改标注通过 `POST /__annotation_save__`（兼容旧路径 `POST /__dev/annotation/save`）写回磁盘，由该插件在 dev server 本地拦截并写入 `public/annotations/`。路径不与常见的 `/api`、`/admin` 等 proxy 前缀冲突。

> 注：非 Vite 构建工具（Webpack/Rspack 等）需自行实现等价的 `POST /__annotation_save__` 开发中间件，逻辑参考 `viteAnnotationPlugin.ts`（同步挂中间件、净化文件名防路径遍历、写入 `public/annotations/`）。

**加载/保存路径说明（通用性要点）：**
- 加载：`{import.meta.env.BASE_URL}annotations/{fileName}.json` —— 自动适配项目配置的 vite `base`（如 `/sub-path/`），不要写死 `/annotations/`
- 保存：优先 `POST /__annotation_save__`，失败再试 `POST /__dev/annotation/save`（兼容旧项目）；成功看 `res.ok`，失败须抛错并由 Overlay 提示，禁止静默吞掉
...
| ⑦ 路径已适配 | 加载用 `BASE_URL`；保存双路径且插件**同步**挂中间件（`enforce: 'pre'`，非 post hook） | 保存 404 / 刷新丢失 |
...
```
【角色】annotation-prd-analyzer
【定义】.agents/agents/annotation-prd-analyzer.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
...
```
【角色】annotation-code-locator
【定义】.agents/agents/annotation-code-locator.md
【交付模式】{DELIVERY_MODE}
【执行】能独立派发则派发；否则主会话扮演。
【输入 / Prompt】
交付模式：{DELIVERY_MODE}
...
> 该角色读代码、识别 DOM 区域、分配唯一 class、输出精确的 Edit 指令。
...
若本步改了 `src/` 或组件文件：宣称完成前按 `.agents/rules/soft-gates.md` 补跑项目验证，JSON 校验不够单独充当完成证据。
...
5. **详情面板可拖动**：`AnnotationPanel` 在宽屏/窄屏均为浮动窗，拖标题栏移动（Pointer 事件支持触控）；禁止再锁成不可拖的底部抽屉
6. **页面接入标注**：全局已挂载 `<AnnotationOverlay />` 时，在路由写入 JSON 文件名（Vue：`meta.annotationFile`；React：`handle.annotationFile`，如 `report-inventory`），对应 `public/annotations/{file}.json`；嵌套路由勿只依赖相对 path（会误解析成 `inventory`）
7. **保存中间件同步挂载**：`viteAnnotationPlugin` 禁止用 configureServer post hook，须 `enforce: 'pre'`；否则编辑「确定」后 JSON 不落盘
