...
4. 生成标注 JSON 数据
5. 保存到 `public/annotations/customer-archive.json`
...
2. 点击页面任意位置添加标注点
3. 填写标注内容后点编辑器「确定」——会 `POST /__annotation_save__` 写入 `public/annotations/{page}.json`
4. 再点底部「退出」离开编辑模式（「退出」本身不负责落盘）

详情面板：拖动**标题栏**可移动位置（窄屏/手机预览同样支持）；边缘可调整大小。

若「确定」后 JSON 未更新：确认 `vite.config.ts` 已注册 `annotationPlugin()`，且已重启 `npm run dev`（插件挂载时机错误时 `/__annotation_save__` 会 404）。

## 标注数据示例

```json
{
  "page": "/customer/archive",
  "title": "客户档案",
  ...
  "annotations": [
    {
      "id": "ann-001",
      "type": "selector",
      "selector": ".filter-card .ant-input[placeholder='客户名称']",
      "position": { "x": 0, "y": 0 },
      "title": "客户名称筛选",
...
```
给 /customer/archive 页面生成标注，参考 docs/01-需求与规划/示例科技客户关系管理模块-SRS需求规格说明书-V1.0.md
```
