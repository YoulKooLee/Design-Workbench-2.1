...
> 第 0 项 = 模板级固定开销，第 1-6 项 = 项目级可变开销，分开设限（第 0 项超 → 瘦身本文件；1-6 超 → 压缩条目粒度）；**不得删第 0 项**。
...
**【阶段滑窗】第二维：按工作流阶段换切片（需求→设计→开发→测试验收，对应 phase1-4）**

10. **阶段知识**：只读当前阶段 `.agents/knowledge/phaseN-*/` 中本任务所需 1-2 个文件，离开即卸载
11. **阶段接力件 = 基线三件套相应章节**：上一阶段交付物即下一阶段输入契约；跨阶段只靠三件套接力，不重读上一阶段过程讨论（路径见「基线记忆维护」）

单次装配 = 基线（0-6）+ 阶段滑窗（10-11）+ 模块滑窗（7-9）≤ 120 KB；阶段内多模块并行时阶段滑窗只装配一次

**【按需检索】永不常驻**

12. 过程讨论 / 历史会话 / 被否决方案（过程层，见「写入侧三态门禁」）
13. 其他模块 / 其他阶段的设计细节（靠三件套与快照接力）
14. `src/themes/` 全量、组件库全量、`vendor/`（禁整目录 Read，见预算铁律）
...
* **组件真源**：原型层 React 组件以 `src/component-templates/`（六个分类：业务组件 / 反馈 / 基础 / 布局 / 数据展示 / 表单，各有 `index.html` + `index.tsx`）为准；`03-组件库` 是母版 / catalog（JSON + 提示词），运行时按白名单拷贝、**禁止整目录 Read**。
...
|开发交付|SRS 落盘 + `SPEC\_SOURCE=SRS`；PRD→SRS 转写门禁通过|`req-doc` Step F / `.agents/rules/prd-to-srs-gate.md`|
...
> 下表仅概括用途；触发条件以注入的 INDEX.md description 为准（见「上下文预算」），严禁为查触发词 Read SKILL.md。唯一例外：需求文档三体系的裁决见下一章专章。
...
各技能须在步骤 1（或等效门禁）记录 `DELIVERY\_MODE`；子 Agent prompt 首行传入 `交付模式：{DELIVERY\_MODE}`。
...
**并行派发** — 无依赖的独立操作必须同一条消息并行（如 3+ 失败因不同根因）；失败相互关联、需完整系统状态、会互改同一文件时不并行。Prompt 聚焦且自包含、明确输出；派发后读摘要 → 查冲突 → 跑完整测试。复杂问题可拆需求/可行性/安全/一致性多视角子 Agent。
...
* **SRS = 需求规格说明书**，研发链路唯一规格真源（`page-generator` / `hld-design` / `lld-design` / `feature-list` / `annotation` / `delivery-plan` 仅以 SRS 为准）。
* **PRD** 是产品探索真源，**不可**直接驱动研发技能（`.agents/rules/prd-to-srs-gate.md` 门禁）；系统架构 + 数据模型 = HLD 的两章节，不新建文档。
...
1. **WORKSPACE\_PATH** = 仓库根目录（含 `.agents/`、`AGENTS.md`）
2. **PROJECT\_PATH** = `admin/`
...
|**基线三件套**|`docs/`（技能权威输入输出路径，见下表「基线记忆维护」）|SRS / HLD / LLD|**项目基线记忆本体**，跑题判定标准；每份头部登记版本+变更时间；跨阶段/跨端只靠它接力|
|需求文档层|`docs/` 其余 + `src/resources/`|需求清单 / 概念版 / PRD / 标书 / 盘点|PRD 是产品探索真源；`src/resources/` 内文档供 Make 预览（`?doc=` 信号）|
|辅助层|`src/resources/` 其他|素材、画布等|支撑基线，不参与跑题判定|
...
|件|定义|落盘路径|产出技能|
|-|-|-|-|
|**SRS**|需求规格说明书（做什么，研发唯一规格真源）|`docs/01-需求与规划/*SRS*.md` 或 `docs/**/*需求*说明书*.md`|`req-doc`|
|**HLD**|概要设计（系统架构 + 数据模型）|`docs/02-架构与设计/*概要设计*.md`|`hld-design`|
|**LLD**|详细设计（逐模块实现）|`docs/02-架构与设计/*详细设计*.md`|`lld-design`|

**维护规则**：

1. **写入门禁**：只有 `[事实]` 类内容可进三件套；过程讨论一律进 `project-memory.md` 过程层（见「写入侧三态门禁」）。
2. **版本登记**：每次修改更新头部版本号 + 变更时间 + 一句话摘要，并在 `project-memory.md` 索引层登记一行；SRS 修改须过 `req-doc-workflow.md` 规范扫描。
...
1. **影响基线三件套？**（SRS/HLD/LLD 任一条，PRD/概念版按需求文档层处理）→ 按「基线记忆维护」回写并登记索引
...
* **来源标注**：PRD/SRS 每条需求带来源（客户原话 / 讨论结论 / 补全待确认）；`[智能体补全待确认]` 不得作为 SPEC\_SOURCE 真源。
