...
import { MobileFrameIos } from './index';

const NAME = 'MobileFrameIos';
const DESC = 'iOS 设备壳（四形态 + 状态栏 + 标签栏）';
const PROMPT = '【组件引用】MobileFrameIos iOS 设备壳（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { MobileFrameIos } from "../../component-templates/页面模式/MobileFrameIos"\n参数：variant（plain|header|nav|tabs）；title；leftText；rightText；navItems；tabs；width；children\n场景：把移动端页面/小程序页面装进 iPhone 外框做演示。页面级组件，直接整页使用。';
...
function App() {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1400, margin: '0 auto' }}>
      ...
      <DemoBoundary><MobileFrameIos /></DemoBoundary>
