// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageListMedia
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageListMedia, MOCK_MEDIA_ASSETS, MOCK_MEDIA_FOLDERS } from './index';

const NAME = 'PageListMedia';
const DESC = '文件管理（目录树 + 宫格/列表 + 星标 + 上传）';
const PROMPT = '【组件引用】PageListMedia 文件管理（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageListMedia } from "../../component-templates/页面模式/PageListMedia"\n参数：assets；folders；pageSize；currentUser\n场景：素材/文件库（目录树 + 宫格或列表视图 + 星标 + 上传与新建目录）。页面级组件，直接整页使用。';
