// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageDashboardAnalytics
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageDashboardAnalytics } from './index';

const NAME = 'PageDashboardAnalytics';
const DESC = '经营分析看板（KPI + 流量质量 + 实时访客 + 页面表现 + 来源）';
const PROMPT = '【组件引用】PageDashboardAnalytics 经营分析看板（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageDashboardAnalytics } from "../../component-templates/页面模式/PageDashboardAnalytics"\n场景：经营/流量分析看板（多 Tab、周期切换、实时访客、页面表现与来源拆解）。页面级组件，直接整页使用。';
