...
import { PageListCard } from './index';

const NAME = 'PageListCard';
const DESC = '标准 / 横版 / 封面 / 服务 四种卡片网格列表（Tabs 切换 + 分页）';
const PROMPT = '【组件引用】PageListCard 卡片列表页（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageListCard } from "../../component-templates/页面模式/PageListCard"\n参数：title；desc；items；serviceItems；defaultPageSize；onCardClick；onServiceAction\n场景：卡片式内容列表（四形态 Tab 切换）。页面级组件，直接整页使用。';
