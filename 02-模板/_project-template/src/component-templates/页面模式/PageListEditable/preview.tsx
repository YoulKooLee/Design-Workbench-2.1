// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageListEditable
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageListEditable } from './index';

const NAME = 'PageListEditable';
const DESC = '行内编辑（就地改字段 / 保存 / 删除 / 列设置 / 导出）';
const PROMPT = '【组件引用】PageListEditable 行内编辑（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageListEditable } from "../../component-templates/页面模式/PageListEditable"\n参数：rows；pageSize\n场景：工单/工单类列表的行内编辑（6 字段就地修改、批量保存、翻页前拦截未保存）。页面级组件，直接整页使用。';
