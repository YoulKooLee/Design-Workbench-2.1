// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageWorkplace
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageWorkplace } from './index';

const NAME = 'PageWorkplace';
const DESC = '工作台首页（KPI + 图表 + 排行 + 快捷入口 + 待办）';
const PROMPT = '【组件引用】PageWorkplace 工作台首页（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageWorkplace } from "../../component-templates/页面模式/PageWorkplace"\n参数：sales；channels；devices；todos；notices；onExport\n场景：中后台首页/数据概览（指标卡、趋势图、来源占比、待办与通知）。页面级组件，直接整页使用。';
