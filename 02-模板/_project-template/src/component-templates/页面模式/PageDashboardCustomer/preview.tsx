// @ts-nocheck
/**
 * 组件预览页 · 页面模式 / PageDashboardCustomer
 */
import React from 'react';
import { createRoot } from 'react-dom/client';
import '../../_kit/theme.css';
import { PageDashboardCustomer } from './index';

const NAME = 'PageDashboardCustomer';
const DESC = '客户洞察看板（KPI + 活动趋势 + 客户列表）';
const PROMPT = '【组件引用】PageDashboardCustomer 客户洞察看板（组件库 页面模式）\n框架：React 18 + Tailwind v4（零第三方依赖）\n用法：import { PageDashboardCustomer } from "../../component-templates/页面模式/PageDashboardCustomer"\n参数：records；pageSize\n场景：客户经营看板（收入/新客/活跃/增长 KPI + 分群趋势 + 客户记录表）。页面级组件，直接整页使用。';
