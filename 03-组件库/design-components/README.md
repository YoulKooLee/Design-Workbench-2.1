...
## 维护真源（2026-09-14 收归共享组件库）

> **本目录（`03-组件库/design-components/`）是组件真源，由豆包统一维护更新。**
> 已登记到 `03-组件库/custom-components.json`（9 条：Button/Card/StatCard/Tag/DataTable/FormField/Modal/ResultPage/theme），工作台「组件库」页签可直接展示 + 复制提示词，**任意项目可读取、复用**。
> 指标管理平台项目内保留引用副本 `src/component-templates/Design-components/`（开发栈「组件」页签继续可用）；真源改动后需同步副本（或直接以共享库提示词引用）。

## 位置与用法（2026-09-14 调整：组件模板独立目录 src/component-templates，区别于开发栈工具组件 src/components）

```
03-组件库/design-components/        ← 共享真源（本目录）
src/component-templates/Design-components/   ← 项目内引用副本
...
组件库卡片浏览入口：`src/component-templates/component-library/`（Make「组件」页签可打开，卡片展示+分类筛选+真实预览+复制提示词）；工作台面板「组件库」页签经 custom-components.json 展示共享条目。
